...
- Egy GitHub repo, ami ~15-20 percenként lekéri a Downdetector adatot (dupla trigger: GitHub Actions cron + cron-job.org)
- Telegram értesítés, ha a bejelentett hibák száma eléri vagy meghaladja a küszöböt (alapértelmezés: 10)
...
A workflow `17,47 * * * *`-vel van ütemezve (GitHub Actions belső cron, `:17` és `:47`-kor – offsetelve a cron-job.org `:00/:30` triggerétől). A GitHub ingyenes cron **pár perces késéssel** szokott indulni. Nézd meg 20 perc múlva az Actions fület, hogy van-e ott új futás magától.

**Opcionális**: cron-job.org külső triggert is beállíthatsz `:00` és `:30`-ra, így ~15 percenként lesz coverage. A setup-hoz fine-grained GitHub PAT kell (Actions write scope, csak erre a repóra), amit a cron-job.org workflow_dispatch API-n keresztül hív.
...
A Downdetector Cloudflare védelmet használ. A jelenlegi megoldás ByParr solver (Docker containerben, GitHub Actions-ben). Ha a solver nem tudja megoldani a challenge-et, az auto-remediation ZenRows API-val és direkt HTTP kéréssel is próbálkozik.

**Első lépés**: nézd meg a GitHub Actions logokat – a solver health check és a remediation próbálkozások részletesen logolva vannak. Ha tartósan blokkol: fontold meg a `SOLVER_IMAGE` GitHub variable módosítását (ByParr ↔ FlareSolverr csere), vagy a ZenRows kredit feltöltését.
