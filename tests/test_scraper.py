...
class TestFetchHtml:
    ...
    @patch("src.scraper._check_solver_health")
    @patch("src.scraper._solver_fetch")
    def test_fetch_html_solver_error_raises(self, mock_fetch, mock_health, mock_config):
        """Solver fails all retries → raises (ZenRows is now in remediation)."""
        mock_config.MAX_RETRIES = 5
        mock_config.RETRY_BACKOFF_BASE = 2.0
    ...
    def test_health_check_fails_raises(self, mock_get, mock_config):
        """Solver unreachable → FetchError (ZenRows is now in remediation)."""
        mock_config.FLARESOLVERR_URL = "http://localhost:8191/v1"
